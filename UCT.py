...
class Node:
    ...
    """
    def __init__(self, acts, move = None, parent = None, state = None):
    ...
    def UCTSelectChild(self):
        ...
        """
        s = sorted(self.childNodes, key = lambda c: c.ret + sqrt(2*log(self.visits)/c.visits))[-1]
        return s
...
def UCT(rootstate, itermax, terminal, verbose = False, cons = []):
    ...
    for i in range(itermax):
        ...
        while not terminal(state): # while state is non-terminal
...
def MCTS(S, A, r, T, s0, terminal, gamma=1, zeroConstraints=[], positiveConstraints=[], positiveConstraintsOcc=1):
    """ Play a sample game between two UCT players where each player gets a different number 
        of UCT iterations (= simulations = tree nodes).
    """
    state = s0
    while terminal(state):
        action = UCT(rootstate = state, itermax = 1000, terminal = terminal, verbose = False, cons = zeroConstraints)
        print state, action
        state = move(state, action, T)
