...
def parseOptions():
    ...
    optParser.add_option('-b', '--k1',action='store', type=float,
                         dest='k1',default=0,
                         help='k1 for Rmax')
...
if __name__ == '__main__':
  ...
  retFile = open(opts.agent + "_" + str(mdp.count) + "_" + str(mdp.stackNum) + "_" +str(opts.k1) + "_result", "a")
  ...
  for episode in range(1, opts.episodes+1):
    ...
    if episode % interval == interval - 1:
      retFile.write(str(episode) + ' ' + str(1.0 * returns / episode) + '\n')
