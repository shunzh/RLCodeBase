...
class TGLearningAgent(ReinforcementAgent):
  ...
  def __init__(self, **args):
    ...
    self.root = QTreeNode(.1, self.predicate.getPossible()) # FIXME epsilon should passed via command line
  ...
  def getQValue(self, state, action):
    ...
    """
    "*** YOUR CODE HERE ***"
    return self.root.lookup(self.getPred(state, action))
  ...
  def getPolicy(self, state):
    ...
    for action in actions:
      sample = self.getPred(state, action)
      curiosity = len(sample) - self.root.compare (sample)

      if self.visits[state, action] >= self.k1 or curiosity == 0:
  ...
  def update(self, state, action, nextState, reward):
    ...
    self.root.update(self.getPred(state, action), new_qvalue)

  def getPred(self, state, action):
    """
      Return the predicate that describes this state
    """
    ret = self.predicate.getPred(state, action)
    #print state, ret
    return ret
...
class BlocksworldPred (PredicateFactory):
  ...
  def getPossible (self):
    return [('on', i, j) for i in range(self.blockNum) for j in range(self.blockNum) + ['floor']]\
         + [('move', i, j) for i in range(self.blockNum) for j in range(self.blockNum) + ['floor']]\
         + [('clear', i) for i in range(self.blockNum) + ['floor']]
  ...
  def getPred (self, state, action):
    ...
    # include action
    #print state, action
    sourceBlock = state[action[0]][-1]
    if state[action[1]] == ():
      targetBlock = 'floor'
    else:
      targetBlock = state[action[1]][-1]
    pred.append(('move', sourceBlock, targetBlock))
...
# a node on a Q-tree
class QTreeNode ():
  def __init__ (self, epsilon, possibleTests):
    self.predicate = None
    self.stat = util.Counter()
    self.epsilon = epsilon # significance (not exploration!)
    self.possibleTests = possibleTests
  ...
  def checkSplit (self):
    ...
    for test in self.possibleTests:
      ...
      splitted = fTest (positive) + fTest (negative)
      joint = fTest (positive + negative)

      #print test, splited, joint
      #print "positive:", positive
      #print "negative:", negative

      if splitted < joint - self.epsilon:
        self.predicate = test
	
        # not initialize the subnodes
	self.leftNode = QTreeNode (self.epsilon, self.possibleTests)
	self.rightNode = QTreeNode (self.epsilon, self.possibleTests)
	self.stat = None
	break
  ...
  def compare (self, sample):
    if self.predicate == None:
      ...
      return ans + max(self.leftNode.compare(sample), self.rightNode.compare(sample))

  def printNode (self, level):
    print "  " * level, # indent

    if self.predicate == None:
      print self.lookup (None)
    else:
      print self.predicate
      self.leftNode.printNode (level + 1)
      self.rightNode.printNode (level + 1)
